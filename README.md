...
Firstly, pair the ADB server with a device:
...
Sometimes Termux:Boot will cease to run the script for whatever reason, just add another script in the same directory:
```
#!/bin/bash
bash $(dirname $0)/start_shizuku.sh
```
